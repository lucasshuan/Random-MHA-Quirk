 ...
 * Atualiza research/manual-copy.json com as descrições EN melhoradas.
 * Após rodar: `pnpm research:build-catalog` para regenerar os arquivos TS.
 */
...
function loadWikiTitles() {
  const index = JSON.parse(readFileSync(join(research, 'wiki-index.json'), 'utf8'))
  const map = new Map()
...
async function main() {
  ...
  console.log('Run: pnpm research:build-catalog to regenerate TS files.')
}
